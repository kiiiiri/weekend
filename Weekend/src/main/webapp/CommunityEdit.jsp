...
<!-- 수정 폼 -->
    <form name="writeForm" method="post" enctype="multipart/form-data"
        action="<%= request.getContextPath() %>/community/edit.do"
        onsubmit="return validateForm(this);">

    <div class="max-w-3xl mx-auto mt-10 p-8 bg-white shadow-lg rounded-2xl">
      <h2 class="text-3xl font-bold mb-6 text-gray-800">게시글 수정</h2>

<input type="hidden" name="prevOfile" value="${dto.cofile}" />
<input type="hidden" name="prevSfile" value="${dto.csfile}" />
...
<div class="flex gap-4 mb-4">
  <input type="text" name="cwuser" readonly
         value="${dto.cwuser}"
         class="bg-gray-200 w-1/2 border rounded-md p-3" required />

  <input type="password" name="cpw" readonly
         value="${dto.cpw}"
         class="bg-gray-200 w-1/2 border rounded-md p-3" required />
</div>
	...
	<input type="hidden" name="imageFiles" id="imageFiles" />
     ...
     <!-- 버튼 영역 -->
		<div class="flex justify-center items-center space-x-2 mt-6">
  		...
  		<button type="button"
          	onclick="location.href='<%= request.getContextPath() %>/community/cview.do?cno=${dto.cno}';"
...
<script>
  ...
  const initialContent = `${dto.ctext}`
